                          ...
                          const Vector3f& dir, float& tnear, float& u, float& v)
{
    ...
    float t_temp = dotProduct(S2, E2) / temp;
    float u_temp = dotProduct(S1, S) / temp;
    float v_temp = dotProduct(S2, dir) / temp;

    if(t_temp >= -epsilon && u_temp >= -epsilon && v_temp >= -epsilon && (1.f - u_temp - v_temp) >= -epsilon) {
        tnear = t_temp;
        u = u_temp;
        v = v_temp;
        return true;
